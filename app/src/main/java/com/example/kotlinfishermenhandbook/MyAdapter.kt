...
class MyAdapter (listArray:ArrayList<ListItem>, context: Context): RecyclerView.Adapter<MyAdapter.ViewHolder>() {
    ...
    class ViewHolder(view: View): RecyclerView.ViewHolder(view) {
        val tvTitle1 = view.findViewById<TextView>(R.id.tvTitle)
        val tvContent1 = view.findViewById<TextView>(R.id.tvContent)
        val im1 = view.findViewById<ImageView>(R.id.im)
        fun bind( listItem:ListItem, context: Context)
        {
            tvTitle1.text = listItem.titleText
            var textCon = listItem.contentText.substring(0,50) + "..."
            tvContent1.text = textCon
            im1.setImageResource(listItem.image_id)
            itemView.setOnClickListener(){
                Toast.makeText(context, "Pressed : ${tvTitle1.text}",Toast.LENGTH_SHORT).show()
                val i = Intent(context,ContentActivity::class.java).apply {
                    putExtra("title", tvTitle1.text.toString())
